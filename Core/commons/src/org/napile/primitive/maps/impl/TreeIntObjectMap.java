...
public class TreeIntObjectMap<V> extends AbstractIntObjectMap<V> implements NavigableIntObjectMap<V>, Cloneable, java.io.Serializable
{
	 ...
	 */
	static <V> int keyOrNull(Entry<V> e)
	{
	...
	static abstract class NavigableSubMap<V> extends AbstractIntObjectMap<V> implements NavigableIntObjectMap<V>, java.io.Serializable
	{
		...
		{
			Entry<V> e = (fromStart ? m.getFirstEntry() : (loInclusive ? m.getCeilingEntry(lo) : m.getHigherEntry(lo)));
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final Entry<V> absHighest()
		{
			Entry<V> e = (toEnd ? m.getLastEntry() : (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
		...
		final Entry<V> absCeiling(int key)
		{
			...
			Entry<V> e = m.getCeilingEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}
		...
		final Entry<V> absHigher(int key)
		{
			...
			Entry<V> e = m.getHigherEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}
		...
		abstract Entry<V> subCeiling(int key);

		abstract Entry<V> subHigher(int key);

		abstract Entry<V> subFloor(int key);

		abstract Entry<V> subLower(int key);
